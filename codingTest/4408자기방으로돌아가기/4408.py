...
for tc in range(1, T+1) :
    ...
    for _ in range(N) :
        pos_room, next_room = map(int, input().split())

        if pos_room > next_room :
            pos_room, next_room = next_room, pos_room

        s = pos_room
        e = next_room
        
        if pos_room%2 == 0 : #출발 방이 짝수면 홀수방도 포함
            s = pos_room - 1 

        if next_room%2 == 1 : #도착 방이 홀수면 짝수방도 포함
            e = next_room + 1
            
        for c in range(s, e+1) :
                count[c] += 1

        # print((pos_room, next_room), s, e)
