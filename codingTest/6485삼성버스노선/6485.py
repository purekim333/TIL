...
for tc in range(1, T+1) :
    ...
    for _ in range(N) :
        ...
        tmp_lst = list(range(a,b+1))
        bus_range.append(tmp_lst)
    ...
    max_n = 5000
    bus_stop_count = [0] * (max_n+1)

    for bus_range_i in bus_range :
        for stop in bus_range_i :
            bus_stop_count[stop] += 1
    
    # for i in range(P) :
    #     print(bus_stop_count[bus_stop[i]])

    # print(bus_stop_count)
    answer = []
    for i in range(P) :
    
        answer.append(bus_stop_count[bus_stop[i]])
