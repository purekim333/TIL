...
def dfs(ability_sum, player) :
    global mx_sum
    
    if player > 10 :
        if mx_sum < ability_sum :
            mx_sum = ability_sum
        return 
    
    for position in range(11):
        if visited[position] == 1 : continue
        if arr[player][position] == 0 : continue
        visited[position] = 1
        ability_sum += arr[player][position]
        dfs(ability_sum, player+1)
        ability_sum -= arr[player][position]
        visited[position] = 0
...
for tc in range(C) :
    # arr 안에 (0 ~ 10) 각 선수들의 능력치를 배열로 저장
    mx_sum = 0
